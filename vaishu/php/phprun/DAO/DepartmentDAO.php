<?php
  ...
  class DepartmentDAO implements DAO{
    ...
    function getdepartmentstatus($departmentName) {
      foreach ($departmentName as $select)
      { 
        $query = "SELECT can_have_multiple_department FROM department where dept_no = '$select'"; 
        $result[] =  $this->db->getdepartmentstatus( $query );
      }
      return $result;
